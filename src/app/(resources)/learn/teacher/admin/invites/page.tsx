...
import { AdminTeacherInvitesClient } from "@/components/platform/AdminTeacherInvitesClient";
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 space-y-6">
        <header className="space-y-3">
          <div className="flex flex-wrap items-center gap-3 text-sm font-semibold text-black/80 dark:text-white/80">
            ...
            <span className="text-black/40 dark:text-white/40">•</span>
            <span>Tutor invites</span>
          </div>
          <h1 className="text-4xl font-extrabold tracking-tight text-black dark:text-white">Tutor invites</h1>
          <p className="max-w-2xl text-sm text-black/70 dark:text-white/70">
            Generate one-time invite codes and optionally send them by email. Admin access required.
          </p>
        </header>

        <AdminTeacherInvitesClient />
      </div>
    </div>
  );
}
