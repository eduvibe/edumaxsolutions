...
export async function POST(req: Request) {
  ...
  const update = parsed.data.suspended
    ? ({ ban_duration: "876000h" } as unknown)
    : ({ ban_duration: "none" } as unknown);

  const { error } = await srv.auth.admin.updateUserById(userId, update as never);
